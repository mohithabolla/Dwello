...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // For demo purposes, allow login even if backend fails (Mock Mode)
      if (err.code === 'ERR_NETWORK') {
        console.warn('Backend not responding, entering session in demo mode');
        onLogin(role);
        navigate('/dashboard');
      }
    } finally {
    ...
    <div className="min-h-screen relative flex items-center justify-center px-6 overflow-hidden bg-dwello-indigo">
      ...
      >
        <div className="bg-dwello-indigo/80 p-10 text-center relative overflow-hidden backdrop-blur-md">
          ...
          <p className="text-dwello-parchment/70 mt-2 font-medium">Access your construction hub</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-6">
          ...
          <button
            type="submit"
            disabled={loading}
            className={`w-full py-5 bg-dwello-indigo text-white font-black text-xs uppercase tracking-[0.2em] rounded-2xl shadow-xl shadow-dwello-indigo/20 hover:bg-dwello-grape hover:scale-[1.02] active:scale-95 transition-all mt-4 ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
          ...
          <div className="text-center pt-4">
            <p className="text-xs font-bold text-dwello-grape/60 dark:text-dwello-lilac">
              New to the platform? <Link to="/signup" className="text-dwello-indigo dark:text-dwello-silk font-black hover:underline uppercase tracking-widest ml-1">Deploy Account</Link>
            </p>
          </div>
        </form>
      </div>
